...
function normalizeActivityType(value) {
  ...
  if (!ACTIVITY_TYPES.has(activityType)) {
    throw validationError('不支持的语音活动类型');
  }
  return activityType;
}
...
export async function savePhoneticsProgressRecord({
  userId,
  activityType,
  score = null,
  accuracy = null,
  ...
  await db.prepare(`
    INSERT INTO phonetics_progress_records
...
export async function getPhoneticsProgressStatsByUserIds(userIds = []) {
  ...
  const rows = await db.prepare(`
    ...
    FROM phonetics_progress_records
  ...
  const stats = {};
  ids.forEach((id) => {
    stats[id] = { sessions: 0, durationMs: 0, averageScore: 0, averageAccuracy: 0, lastPracticedAt: null };
  });
  (rows || []).forEach((row) => {
    stats[row.user_id] = {
...
export async function getPhoneticsProgressStats(userId) {
